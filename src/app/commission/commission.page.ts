...
private translate: TranslateService,public loadingController: LoadingController,private router: Router) 
  {
		...
		this.Globale.traduire('commission');
  }
 ...
 async ngOnInit() {
    ...
    let url = Constants.API_URL+'&tken='+tocken+'&_ajx=commission&pge='+this.page+'&debut='+debut+'&filtre='+filtre+'&fin='+fin+'&user='+user;
   ...
   moveTo(pge)
  {
	...
	let user =  this.activatedRoute.snapshot.paramMap.get('user');
	let variable = {page:1,debut:this.Resultat.ddate,fin:this.Resultat.fdate,user:user,time:n};
	switch(pge)
  	{
  		
		case 'deb':
			variable.page = 1
  			this.moveToPage('/commission',variable);
  		break;
  		case 'next':
  ...
  actualise(searchForm: NgForm)
	{
		...
		let user =  this.activatedRoute.snapshot.paramMap.get('user');
		this.moveToPage('/commission',{page:1,debut:this.Resultat.ddate,user:user,fin:this.Resultat.fdate,time:n});
	}
  	moveToPage(page,obj)
	{
