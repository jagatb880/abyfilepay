...
@Component({
  selector: 'app-afric-recap',
  templateUrl: './afric-recap.page.html',
  styleUrls: ['./afric-recap.page.scss'],
})
export class AfricRecapPage implements OnInit { Globale;
...
private translate: TranslateService,
	...
	{ 
		...
		this.Globale.traduire('africrecap');
	}
	...
	async ngOnInit() 
	{
		...
		let url = Constants.API_URL+'&tken='+tocken+'&_ajx=afric&numero='+this.activatedRoute.snapshot.paramMap.get('number')+'&montant='+this.activatedRoute.snapshot.paramMap.get('montant')+'&opera='+this.activatedRoute.snapshot.paramMap.get('opera')+'';
  ...
  async onSubmit(loginForm: NgForm)
  {
    ...
    const loading = await this.loadingController.create({
        message: this.Globale.Translate.africmomo,
        duration: 20000
      });
      this.presentLoading(loading);
    //Création du lien Json pour créer un compte 
    let tocken = this.Globale.Session.tocken;
    let url = Constants.API_URL+'&tken='+tocken+'&_ajx=argent&numero='+this.number+'&country='+this.activatedRoute.snapshot.paramMap.get('opera')+'&montant='+this.montant+'&opera=momoafric&cash=IN';
    let result = this.http.get(url);
    result.subscribe (data => //En cas de succès de la requête
    {
      		if(data['error']==1)
