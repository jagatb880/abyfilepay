...
export class ReloadPage implements OnInit { Globale;
...
async presentOperator(infos) {
  ...
  {
		if(this.isnumber) this.moveToPage('/reload-recap',{opera: this.opera.code,pays: this.pays.code,montant: verifForm.controls['montant'].value,number:verifForm.controls['number'].value});
